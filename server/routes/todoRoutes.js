...
router.patch('/done/:id', async (req, res) => {
    try{
        ...
        const data = await Todo.findById(id, {owner: ownerId});
        const new_value = !data.done;
        await Todo.findByIdAndUpdate(id, {done: new_value});
        res.status(200).send("Uddated done");
    }
    catch(error){
        res.status(500).json({message: error.message})
    }
